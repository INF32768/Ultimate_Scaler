...
public abstract class MixinShiftedNoiseSampler {
    ...
    private void modifyNoiseSampleArgs(Args args, DensityFunction.NoisePos pos) {
        ...
        double d = (pos.blockX() * config.globalXScale + config.globalXOffset) * this.getXzScale() + this.getShiftX().sample(pos);
        double e = (pos.blockY() * config.globalYScale + config.globalYOffset) * this.getYScale() + this.getShiftY().sample(pos);
        double f = (pos.blockZ() * config.globalZScale + config.globalZOffset) * this.getXzScale() + this.getShiftZ().sample(pos);
