...
public abstract class MixinShiftedNoiseSampler {
    ...
    private void modifyNoiseSampleArgs(Args args, DensityFunction.NoisePos pos) {
        ...
        double d = (pos.blockX() * WorldGenOptions.globalScale[0] + WorldGenOptions.globalOffset[0]) * this.getXzScale() + this.getShiftX().sample(pos);
        double e = (pos.blockY() * WorldGenOptions.globalScale[1] + WorldGenOptions.globalOffset[1]) * this.getYScale() + this.getShiftY().sample(pos);
        double f = (pos.blockZ() * WorldGenOptions.globalScale[2] + WorldGenOptions.globalOffset[2]) * this.getXzScale() + this.getShiftZ().sample(pos);
